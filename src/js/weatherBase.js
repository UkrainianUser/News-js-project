...
export async function addWeather() {
  try {
    ...
    <div class="weather__block"><span class="weather__number">${Math.round(
      data.main.temp
    )}&#176;</span>
      <div class = "weather__location"><h2 class="weather__title">${
        data.weather[0].main
      }</h2>
      ...
      <p class="weather__date">${new Date(data.dt * 1000).toLocaleDateString(
        'en-GB',
        options
      )}</p></li>`;
